...
if(($nombre != "") && ($apellido != "") && ($correo != "") && ($direccion != "") && ($telefono != "") && ($escuelaID != "") && ($facebook != "") && ($whatsapp != "") && ($contrasena != "")){
    $resultado = maestro::regmaestro($nombre, $apellido, $correo, $direccion, $telefono, $escuelaID, $facebook, $whatsapp, $contrasena); //Realizamos la ejecución de nuestra funcion para registrar escuela, pasandole los datos que recibimos anteriormente
    if ($resultado==1) { //Comprobamos que recibimos un 1 como señal de que se registro la escuela
        ...
        $json_string = json_encode(array("estado" => 1,"mensaje" => "Maestro registrado correctamente.")); //Preparamos el mensaje a enviar como respuesta
